...
import { getUserWallet, UserWallet, refreshWalletBalance } from '@/services/fcmb.service';
...
export default function WalletPage() {
    ...
    const handleRefreshBalance = async () => {
        ...
        try {
            ...
            toast({ title: 'Balance Updated', description: `Your new balance is $${newBalance.toFixed(2)}` });
        } catch (error) {
            console.error("Failed to refresh balance:", error);
            toast({ variant: 'destructive', title: 'Error', description: 'Could not refresh balance.' });
        } finally {
        ...
        return (
            <Card className="my-0">
                <CardHeader>
                    ...
                    <CardDescription>We couldn't find a wallet associated with your account. This might be because your KYC is pending or was rejected. Please contact support.</CardDescription>
                </CardHeader>
            </Card>
        );
    }
            ...
            <Card className="my-0">
                <CardHeader>
                    <div className="flex justify-between items-start">
                        ...
                        <Button onClick={handleRefreshBalance} variant="outline" size="sm" disabled={isRefreshing}>
                ...
                <CardContent className="grid md:grid-cols-2 gap-6">
                    <div className="bg-primary text-primary-foreground p-6 rounded-lg flex flex-col justify-between">
                        <div>
                            ...
                            <p className="text-4xl font-bold font-headline tracking-wider">${wallet.balance.toLocaleString()}</p>
                        </div>
                        <div className="flex gap-2 mt-4">
                            <Button variant="secondary"><ArrowUp className="mr-2 h-4 w-4" /> Withdraw Funds</Button>
                        </div>
                    </div>
                     <div className="bg-muted p-6 rounded-lg">
                        <h3 className="font-semibold mb-4">Your Funding Account</h3>
                        <p className="text-sm text-muted-foreground mb-2">To deposit funds, transfer money to this account:</p>
                        <div className="space-y-3">
                            <div className="flex items-center justify-between p-3 bg-background rounded-md">
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <Button size="icon" variant="ghost" className="h-7 w-7" onClick={() => copyToClipboard(wallet.accountNumber)}>
