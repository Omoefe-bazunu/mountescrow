...
export default function DashboardPage() {
    ...
    const [stats, setStats] = useState<DashboardStats>({ activeDeals: 0, pendingProposals: 0 });
    ...
    const fetchDashboardData = async () => {
        ...
        try {
            ...
            const dealsQuery = query(
                ...
                where(user.uid ? 'buyerId' : 'sellerEmail', '==', user.uid || user.email)
            );
        ...
        <Card>
            ...
            <CardContent>
                ...
                <p className="text-xs text-muted-foreground">
                    {wallet?.updatedAt ? `Updated ${formatDistanceToNow(toDate(wallet.updatedAt)!)} ago` : 'Available funds'}
                </p>
            </CardContent>
        </Card>
        <Card>
            ...
            <CardContent>
                <div className="text-2xl font-bold font-headline tracking-wider">{stats.activeDeals}</div>
                <p className="text-xs text-muted-foreground">Deals awaiting funding or in progress</p>
            </CardContent>
        </Card>
        <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <CardContent>
                <div className="text-2xl font-bold font-headline tracking-wider">{stats.pendingProposals}</div>
      ...
      <Card>
        <CardHeader className="flex flex-row items-center">
            ...
            <Button asChild size="sm" className="ml-auto gap-1">
                <Link href="/transactions">View All <ArrowUpRight className="h-4 w-4" /></Link>
            </Button>
        </CardHeader>
        <CardContent>
              ...
              <TableBody>
                {transactions.map(tx => (
                  <TableRow key={tx.id}>
                    ...
                    <TableCell>
                      {toDate(tx.createdAt) ? formatDistanceToNow(toDate(tx.createdAt)!) + ' ago' : 'N/A'}
                    </TableCell>
                    <TableCell className="text-right font-medium">
                      {['DEPOSIT', 'MILESTONE_PAYMENT'].includes(tx.type) ? '+' : '-'}${tx.amount.toFixed(2)}
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
