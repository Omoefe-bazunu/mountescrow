...
import { getDisputes, DisputeData } from '@/services/dispute.service';
import { CreateDisputeDialog } from './_components/create-dispute-dialog';
import { Skeleton } from '@/components/ui/skeleton';
import { format } from 'date-fns';

export default function DisputesPage() {
    const [disputes, setDisputes] = useState<({ id: string } & DisputeData)[]>([]);
            ...
            <Card className="my-0">
                <CardHeader className="flex flex-row items-center justify-between">
                    <div>
                        ...
                        <CardDescription>View and manage your transaction disputes.</CardDescription>
                    </div>
                                    ...
                                    <TableRow key={dispute.id}>
                                        ...
                                        <TableCell>
                                            <div>
                                                ...
                                                <div className="text-sm text-muted-foreground">{dispute.dealId}</div>
                                            </div>
                                        </TableCell>
                                        <TableCell>
                                        ...
                                        <TableCell className="text-right">
                                            <Button variant="ghost" size="icon" asChild>
                                                <Link href={`/disputes/${dispute.id}`}><ArrowRight className="h-4 w-4" /></Link>
                                            </Button>
                                        </TableCell>
                                    </TableRow>
                                ))}
