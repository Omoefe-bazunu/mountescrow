...
import { collection, addDoc, serverTimestamp, query, where, getDocs, doc, getDoc, or, updateDoc, writeBatch } from 'firebase/firestore';
import type { ProposalData } from './proposal.service';
import { releaseFromEscrow, transferToEscrow, getUserWallet } from './fcmb.service';
...
export interface Milestone {
  ...
  description: string;
  dueDate: any;
  status: 'Pending' | 'Funded' | 'Submitted for Approval' | 'Revision Requested' | 'Completed';
  files?: any[];
  submission?: {
      message: string;
      files: UploadedFile[];
...
export interface DealData extends Omit<ProposalData, 'status' | 'createdAt' | 'updatedAt' | 'milestones'> {
...
export async function createDealFromProposal(proposal: { id: string } & Omit<ProposalData, 'id'>): Promise<string> {
    ...
    try {
        ...
        if (proposal.buyerEmail) {
            try {
                ...
                console.error('Failed to send deal creation notification email:', emailError);
            }
        }
...
export async function getDealById(id: string): Promise<({ id: string } & DealData) | null> {
    const user = auth.currentUser;
    if (!user) throw new Error("User not authenticated");
...
export async function fundDeal(dealId: string, buyerId: string, amount: number) {
    if (!dealId?.trim()) {
    ...
    if (Math.abs(amount - expectedAmount) > 0.01) {
        throw new Error(`Amount mismatch. Expected: $${expectedAmount.toFixed(2)}, Provided: $${amount.toFixed(2)}`);
    }
    ...
    try {
        ...
        console.error('Failed to send deal funding notification email:', emailError);
    }
}


export async function approveAndReleaseMilestone(dealId: string, milestoneIndex: number) {
    if (!dealId?.trim()) {
        throw new Error('Deal ID is required');
    }
    ...
    if (dealData.buyerId !== userId) throw new Error("Only the buyer can approve milestones.");
    if (milestone.status !== 'Submitted for Approval') {
        throw new Error(`Cannot approve milestone. Current status: ${milestone.status}`);
    }

    // Find seller's user ID
    const sellerUserQuery = await getDocs(query(collection(db, 'users'), where('email', '==', dealData.sellerEmail)));
...
export async function fundNextMilestone(dealId: string, completedMilestoneIndex: number) {
    if (!dealId?.trim()) {
        throw new Error('Deal ID is required');
    }
    ...
    if (nextMilestoneIndex < milestones.length) {
        ...
        await updateDoc(dealDocRef, { status: 'Completed', updatedAt: serverTimestamp() });
        ...
        try {
            ...
            console.error('Failed to send deal completion notification emails:', emailError);
        }
    }
}

export async function submitMilestoneWork(dealId: string, milestoneIndex: number, message: string, files: FileList | null) {
    if (!dealId?.trim()) {
        throw new Error('Deal ID is required');
    }
    if (typeof milestoneIndex !== 'number' || milestoneIndex < 0) {
    ...
    if (!['Funded', 'Revision Requested'].includes(milestones[milestoneIndex].status)) {
        throw new Error(`Cannot submit work. Milestone status: ${milestones[milestoneIndex].status}`);
    }
    ...
    if (dealData.buyerEmail) {
        try {
            ...
            console.error('Failed to send milestone submission notification email:', emailError);
        }
    }
}

export async function requestMilestoneRevision(dealId: string, milestoneIndex: number, message: string) {
    if (!dealId?.trim()) {
        throw new Error('Deal ID is required');
    }
    if (typeof milestoneIndex !== 'number' || milestoneIndex < 0) {
    ...
    if (milestones[milestoneIndex].status !== 'Submitted for Approval') {
        throw new Error(`Cannot request revision. Milestone status: ${milestones[milestoneIndex].status}`);
    }
    ...
    try {
        ...
        console.error('Failed to send revision request notification email:', emailError);
    }
}
