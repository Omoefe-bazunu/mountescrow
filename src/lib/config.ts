...
export function validateEnvironmentVariables() {
    ...
    const fcmbAvailable = fcmbConfig.clientId && fcmbConfig.clientSecret && 
                         fcmbConfig.authUrl && fcmbConfig.apiBaseUrl;
    ...
    if (!fcmbAvailable) {
        ...
        console.warn('Will attempt to use provided credentials or fall back to mock mode.');
    }
...
export function validateEmailJSConfig() {
    ...
    if (missing.length > 0) {
        console.warn(`EmailJS configuration incomplete. Missing: ${missing.join(', ')}`);
        return false;
    }
    return true;
}