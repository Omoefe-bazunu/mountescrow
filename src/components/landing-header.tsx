...
export function LandingHeader() {
  ...
  const NavLinks = ({isMobile = false}: {isMobile?: boolean}) => {
    ...
    const wrapperProps: { asChild?: boolean } = isMobile ? { asChild: true } : {};

    const linkClass = "text-foreground/60 transition-colors hover:text-foreground/80";
    const mobileLinkClass = "block w-full text-left p-2 rounded-md hover:bg-muted"
    ...
    return (
        <>
            <Wrapper {...wrapperProps}>
                 <Link href="/why-mountescrow" className={navLinkStyles}>Why Mountescrow?</Link>
            </Wrapper>
            <Wrapper {...wrapperProps}>
                <Link href="/products" className={navLinkStyles}>Products</Link>
            </Wrapper>
            <DropdownMenu>
                <DropdownMenuTrigger className={cn("flex items-center gap-1 outline-none", navLinkStyles)}>
             ...
             <Wrapper {...wrapperProps}>
                <Link href="/fee-calculator" className={navLinkStyles}>Fees</Link>
             </Wrapper>
        </>
    );
  };
